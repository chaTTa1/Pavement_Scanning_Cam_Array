# -*- coding: utf-8 -*-
"""
Created on Tue Apr 21 14:19:06 2026

@author: rqy19
"""

"""
Code B — Original EKF (bugs preserved)
=======================================
Modified ONLY for:
  - Windows/Linux cross-platform serial ports
  - Auto-detect ports
  - Command-line arguments
  - Same output format as Code A for comparison
  - Can run without GPS (IMU-only mode)
  - Buffered CSV logging (so it doesn't freeze at 400 Hz)
  - Timestamp in microseconds (confirmed from hardware)

ALL ORIGINAL EKF BUGS PRESERVED:
  - No F Jacobian (filterpy uses identity)
  - Always subtracts gravity (even though TM151 is gravity-free)
  - Gravity init from near-zero accel data
  - Fixed payload format (requires 9 floats)
  - HDOP used as sigma directly (no UERE scaling)
  - Q not scaled by dt
  - arctan2 GPS conversion
"""

import numpy as np
from filterpy.kalman import ExtendedKalmanFilter
from scipy.spatial.transform import Rotation as R
import serial
import serial.tools.list_ports
import pynmea2
import struct
import time
import csv
import os
import sys
import argparse
from geopy.distance import geodesic
from geopy.point import Point


# ─────────────────────────────────────────────
# PLATFORM DETECTION (same as Code A)
# ─────────────────────────────────────────────

def detect_os():
    if sys.platform.startswith("win"):
        return "Windows"
    elif sys.platform.startswith("linux"):
        return "Linux"
    else:
        print(f"[WARNING] Unknown platform '{sys.platform}', using Linux defaults")
        return "Linux"


def get_default_ports(os_name):
    if os_name == "Windows":
        return "COM12", "COM8"
    else:
        return "/dev/ttyACM2", "/dev/ttyACM0"


def list_serial_ports():
    try:
        ports = serial.tools.list_ports.comports()
        if not ports:
            print("  No serial ports found.")
            return
        for p in ports:
            if p.vid is not None:
                print(
                    f"  {p.device:20s} | "
                    f"VID:PID={p.vid:04X}:{p.pid:04X} | "
                    f"{p.description}"
                )
            else:
                print(
                    f"  {p.device:20s} | "
                    f"VID:PID=None         | "
                    f"{p.description}"
                )
    except Exception as exc:
        print(f"  Error listing ports: {exc}")


def auto_detect_ports(os_name):
    IMU_USB_IDS = [
        (0x0483, 0x5740),
    ]
    GPS_USB_IDS = [
        (0x1546, 0x01A8),
        (0x10C4, 0xEA60),
        (0x067B, 0x2303),
        (0x0403, 0x6001),
        (0x152A, 0x8400),
    ]

    imu_port = None
    gps_port = None

    try:
        ports = serial.tools.list_ports.comports()
    except Exception:
        return None, None

    print("Scanning serial ports...")
    for p in ports:
        vid = p.vid
        pid = p.pid
        if vid is not None:
            print(
                f"  {p.device:20s} | "
                f"VID:PID={vid:04X}:{pid:04X} | "
                f"{p.description}"
            )
        else:
            print(
                f"  {p.device:20s} | "
                f"VID:PID=None         | "
                f"{p.description}"
            )
            continue

        for known_vid, known_pid in IMU_USB_IDS:
            if vid == known_vid and pid == known_pid:
                if imu_port is None:
                    imu_port = p.device
                    print(f"  -> Detected IMU: {p.device}")

        for known_vid, known_pid in GPS_USB_IDS:
            if vid == known_vid and pid == known_pid:
                if gps_port is None:
                    gps_port = p.device
                    print(f"  -> Detected GPS: {p.device}")

    return imu_port, gps_port


def resolve_ports(os_name, args):
    default_imu, default_gps = get_default_ports(os_name)
    auto_imu, auto_gps = auto_detect_ports(os_name)

    imu_port = args.imu_port or auto_imu or default_imu
    gps_port = args.gps_port or auto_gps or default_gps

    imu_source = ("command-line" if args.imu_port
                  else "auto-detected" if auto_imu
                  else "default")
    gps_source = ("command-line" if args.gps_port
                  else "auto-detected" if auto_gps
                  else "default")

    print(f"IMU port: {imu_port} ({imu_source})")
    print(f"GPS port: {gps_port} ({gps_source})")

    return imu_port, gps_port


def parse_args():
    parser = argparse.ArgumentParser(
        description="Code B — Original EKF (bugs preserved)"
    )
    parser.add_argument("--imu-port", type=str, default=None)
    parser.add_argument("--gps-port", type=str, default=None)
    parser.add_argument("--imu-baud", type=int, default=None)
    parser.add_argument("--gps-baud", type=int, default=None)
    parser.add_argument("--no-imu", action="store_true")
    parser.add_argument("--no-gps", action="store_true")
    parser.add_argument("--list-ports", action="store_true")
    return parser.parse_args()


_args = parse_args()

if _args.list_ports:
    print("Available serial ports:")
    list_serial_ports()
    sys.exit(0)

OS_NAME = detect_os()
IMU_ENABLED = not _args.no_imu
GPS_ENABLED = not _args.no_gps
IMU_PORT, GPS_PORT = resolve_ports(OS_NAME, _args)
IMU_BAUD = _args.imu_baud if _args.imu_baud else 115200
GPS_BAUD = _args.gps_baud if _args.gps_baud else 115200


# ─────────────────────────────────────────────
# ORIGINAL CODE B SETTINGS (preserved exactly)
# ─────────────────────────────────────────────

ACCEL_INPUT_IS_G = True          # BUG PRESERVED: original had False
GRAVITY_MPS2 = 9.81
PRINT_PREDICT_DEBUG = True
PRINT_PREDICT_EVERY_N = 400
PRINT_RAW_GPS_DEBUG = True
PRINT_ALL_NMEA_LINES = False
LOG_RAW_NMEA_LINES = True

MIN_FIX_QUALITY = 1
MAX_RAW_GPS_JUMP_M = 15.0
MAX_EKF_POSITION_RADIUS_M = 1000.0
MAX_EKF_ALTITUDE_OFFSET_M = 200.0
MAX_VALID_IMU_DT = 0.005          # Fixed: was 0.1, but need 0.005 for 400 Hz µs timestamps

GPS_FIX_TIMEOUT_S = 30

print(f"OS: {OS_NAME}")
print(f"IMU: {'ENABLED' if IMU_ENABLED else 'DISABLED'} | {IMU_PORT} @ {IMU_BAUD}")
print(f"GPS: {'ENABLED' if GPS_ENABLED else 'DISABLED'} | {GPS_PORT} @ {GPS_BAUD}")
print("[CODE B] All original EKF bugs preserved for comparison")


# ─────────────────────────────────────────────
# ORIGINAL HELPER FUNCTIONS (unchanged)
# ─────────────────────────────────────────────

def quaternion_propagate(q, omega, dt):
    r = R.from_quat(q)
    delta_r = R.from_rotvec(omega * dt)
    new_r = r * delta_r
    return new_r.as_quat()


def normalize_quaternion(q):
    norm = np.linalg.norm(q)
    if norm < 1e-12:
        return np.array([0.0, 0.0, 0.0, 1.0])
    return q / norm


def normalize_angle(angle_rad):
    return (angle_rad + np.pi) % (2.0 * np.pi) - np.pi


def heading_deg_to_yaw_rad(heading_deg):
    return normalize_angle(np.deg2rad(heading_deg))


def quaternion_to_yaw(q):
    q = normalize_quaternion(q)
    r = R.from_quat(q)
    yaw = r.as_euler("zyx", degrees=False)[0]
    return normalize_angle(yaw)


def angle_residual(z, z_pred):
    return np.array([normalize_angle(float(z[0]) - float(z_pred[0]))])


def valid_nmea_checksum(line):
    try:
        if not line.startswith("$") or "*" not in line:
            return False
        data, checksum = line.strip().split("*", 1)
        calc = 0
        for c in data[1:]:
            calc ^= ord(c)
        return calc == int(checksum[:2], 16)
    except Exception:
        return False


def nmea_to_decimal_degrees(value_str, direction, is_latitude):
    if value_str is None or value_str == "":
        return None
    value_str = value_str.strip()
    if len(value_str) < 6:
        return None
    try:
        value = float(value_str)
    except ValueError:
        return None

    degrees = int(value // 100)
    minutes = value - degrees * 100
    decimal = degrees + minutes / 60.0

    if direction in ("S", "W"):
        decimal = -decimal

    if is_latitude and not (-90.0 <= decimal <= 90.0):
        return None
    if not is_latitude and not (-180.0 <= decimal <= 180.0):
        return None

    return decimal


# ─────────────────────────────────────────────
# ORIGINAL fx — BUG: always subtracts gravity
# ─────────────────────────────────────────────

def fx(x, dt, u):
    """
    BUG PRESERVED: Always subtracts gravity even though TM151
    outputs gravity-free linear acceleration.
    """
    pos = x[0:3]
    vel = x[3:6]
    quat = x[6:10]
    b_g = x[10:13]
    b_a = x[13:16]

    gyro = u[:3] - b_g
    acc = u[3:6] - b_a

    if ACCEL_INPUT_IS_G:
        acc = acc * GRAVITY_MPS2

    quat = normalize_quaternion(quat)
    r = R.from_quat(quat)

    acc_world = r.apply(acc)
    acc_world[2] -= GRAVITY_MPS2    # BUG: subtracts gravity from gravity-free data

    new_pos = pos + vel * dt + 0.5 * acc_world * dt**2
    new_vel = vel + acc_world * dt
    new_quat = quaternion_propagate(quat, gyro, dt)

    x_new = np.zeros(16)
    x_new[0:3] = new_pos
    x_new[3:6] = new_vel
    x_new[6:10] = normalize_quaternion(new_quat)
    x_new[10:13] = b_g
    x_new[13:16] = b_a
    return x_new


# ─────────────────────────────────────────────
# ORIGINAL MEASUREMENT MODELS (unchanged)
# ─────────────────────────────────────────────

def hx_position(x):
    return x[0:3]


def H_jacobian_position(x):
    H = np.zeros((3, 16))
    H[:, :3] = np.eye(3)
    return H


def hx_heading(x):
    quat = x[6:10]
    return np.array([quaternion_to_yaw(quat)])


def H_jacobian_heading(x):
    H = np.zeros((1, 16))
    base_yaw = hx_heading(x)[0]
    eps = 1e-6
    for idx in range(6, 10):
        x_perturbed = np.array(x, dtype=float)
        x_perturbed[idx] += eps
        x_perturbed[6:10] = normalize_quaternion(x_perturbed[6:10])
        perturbed_yaw = hx_heading(x_perturbed)[0]
        H[0, idx] = normalize_angle(perturbed_yaw - base_yaw) / eps
    return H


# ─────────────────────────────────────────────
# BUFFERED CSV LOGGER (replaced pandas for speed)
# ─────────────────────────────────────────────

class CSVLogger:
    def __init__(self, filename, buffer_size=200):
        self.filename = filename
        self.buffer_size = buffer_size
        self._buffer = []
        self._fieldnames = None
        self._file_exists = os.path.exists(filename)

    def log(self, data_dict):
        if self._fieldnames is None:
            self._fieldnames = list(data_dict.keys())
        self._buffer.append(data_dict)
        if len(self._buffer) >= self.buffer_size:
            self.flush()

    def flush(self):
        if not self._buffer or self._fieldnames is None:
            return
        write_header = not self._file_exists
        try:
            with open(self.filename, "a", newline="") as f:
                writer = csv.DictWriter(f, fieldnames=self._fieldnames)
                if write_header:
                    writer.writeheader()
                    self._file_exists = True
                writer.writerows(self._buffer)
        except IOError as exc:
            print(f"[LOG ERROR] {exc}")
        self._buffer.clear()

    def __del__(self):
        try:
            self.flush()
        except Exception:
            pass


position_logger = CSVLogger("position_log_B.csv", buffer_size=400)
nmea_logger = CSVLogger("raw_nmea_log_B.csv", buffer_size=100)


def log_position_and_error(
    timestamp, gps_pos, position_source,
    gps_error=None, heading_deg=None,
    raw_gps_lat=None, raw_gps_lon=None, raw_gps_alt=None,
    raw_fix_quality=None, raw_hdop=None,
    imu_dt=None, ekf_pos_local=None, ekf_vel_local=None,
):
    if gps_pos is None and ekf_pos_local is None:
        return

    lat_err, lon_err, alt_err = (None, None, None)
    if gps_error is not None:
        lat_err, lon_err, alt_err = gps_error

    data = {
        "timestamp": timestamp,
        "lat": gps_pos[0] if gps_pos is not None else None,
        "lon": gps_pos[1] if gps_pos is not None else None,
        "alt": gps_pos[2] if gps_pos is not None else None,
        "position_source": position_source,
        "heading_deg": heading_deg,
        "raw_gps_lat": raw_gps_lat,
        "raw_gps_lon": raw_gps_lon,
        "raw_gps_alt": raw_gps_alt,
        "raw_fix_quality": raw_fix_quality,
        "raw_hdop": raw_hdop,
        "lat_err": lat_err,
        "lon_err": lon_err,
        "alt_err": alt_err,
        "imu_dt": imu_dt,
        "ekf_x_m": None,
        "ekf_y_m": None,
        "ekf_z_m": None,
        "ekf_vx_mps": None,
        "ekf_vy_mps": None,
        "ekf_vz_mps": None,
    }

    if ekf_pos_local is not None:
        data["ekf_x_m"] = float(ekf_pos_local[0])
        data["ekf_y_m"] = float(ekf_pos_local[1])
        data["ekf_z_m"] = float(ekf_pos_local[2])

    if ekf_vel_local is not None:
        data["ekf_vx_mps"] = float(ekf_vel_local[0])
        data["ekf_vy_mps"] = float(ekf_vel_local[1])
        data["ekf_vz_mps"] = float(ekf_vel_local[2])

    position_logger.log(data)


def log_raw_nmea(timestamp, line, msg_type=None, parse_status="ok",
                 note=None):
    if not LOG_RAW_NMEA_LINES:
        return
    nmea_logger.log({
        "timestamp": timestamp,
        "raw_nmea": line,
        "msg_type": msg_type,
        "parse_status": parse_status,
        "note": note,
    })


# ─────────────────────────────────────────────
# ORIGINAL EKF CLASS — BUG: no F Jacobian, no Q scaling
# ─────────────────────────────────────────────

class SensorFusionEKF:
    def __init__(self):
        self.ekf = ExtendedKalmanFilter(dim_x=16, dim_z=3)

        # BUG PRESERVED: identity F used for covariance propagation
        self.ekf.x = np.zeros(16)
        self.ekf.x[9] = 1.0  # qw = 1.0

        self.ekf.P = np.eye(16) * 100.0

        # BUG PRESERVED: Q not scaled by dt
        self.ekf.Q = np.eye(16) * 0.1
        self.ekf.Q[0:3, 0:3] = np.eye(3) * 0.01
        self.ekf.Q[3:6, 3:6] = np.eye(3) * 0.1
        self.ekf.Q[6:10, 6:10] = np.eye(4) * 0.01
        self.ekf.Q[10:13, 10:13] = np.eye(3) * 0.001
        self.ekf.Q[13:16, 13:16] = np.eye(3) * 0.01

        self.ekf.R = np.eye(3) * 1.0

        self.reference_gps = None
        self.last_imu_time = None
        self.dt = 0.0025
        self.predict_counter = 0
        self.skipped_predicts = 0

        self.has_gps = False
        self.has_imu = False

    def set_reference_gps(self, lat, lon, alt):
        if alt is None:
            alt = 0.0
        self.reference_gps = (lat, lon, alt)
        self.has_gps = True

    def update_imu(self, gyro, accel, timestamp_us):
        """
        Uses microsecond timestamps (confirmed from hardware).
        """
        if self.last_imu_time is None:
            self.last_imu_time = timestamp_us
            self.has_imu = True
            return False

        dt = (timestamp_us - self.last_imu_time) / 1e6
        self.last_imu_time = timestamp_us

        if dt <= 0 or dt > MAX_VALID_IMU_DT:
            self.dt = 0.0025
            self.skipped_predicts += 1
            return False

        self.dt = dt
        u = np.concatenate([gyro, accel])

        # BUG PRESERVED: lambda captures self.dt by reference
        # BUG PRESERVED: no F Jacobian set — filterpy uses identity
        self.ekf.fx = lambda x: fx(x, self.dt, u)
        self.ekf.predict()
        self.ekf.x[6:10] = normalize_quaternion(self.ekf.x[6:10])

        self.predict_counter += 1

        pos = self.ekf.x[0:3]
        vel = self.ekf.x[3:6]

        if (np.linalg.norm(pos[:2]) > MAX_EKF_POSITION_RADIUS_M
                or abs(pos[2]) > MAX_EKF_ALTITUDE_OFFSET_M):
            print(
                f"[STATE WARNING] EKF diverging: "
                f"pos=({pos[0]:.1f}, {pos[1]:.1f}, {pos[2]:.1f})"
            )

        if (PRINT_PREDICT_DEBUG
                and self.predict_counter % PRINT_PREDICT_EVERY_N == 0):
            print(
                f"[PREDICT] dt={self.dt:.6f} "
                f"pos=({pos[0]:.3f}, {pos[1]:.3f}, {pos[2]:.3f}) "
                f"vel=({vel[0]:.3f}, {vel[1]:.3f}, {vel[2]:.3f})"
            )

        return True

    def update_gps(self, lat, lon, alt, pos_sigma=None):
        """BUG PRESERVED: pos_sigma = hdop directly, no UERE scaling."""
        if self.reference_gps is None:
            return False
        if alt is None:
            alt = self.reference_gps[2]

        try:
            e = geodesic(
                (self.reference_gps[0], self.reference_gps[1]),
                (self.reference_gps[0], lon)
            ).meters
            e *= 1 if lon >= self.reference_gps[1] else -1

            n = geodesic(
                (self.reference_gps[0], self.reference_gps[1]),
                (lat, self.reference_gps[1])
            ).meters
            n *= 1 if lat >= self.reference_gps[0] else -1

            if abs(e) > 1000 or abs(n) > 1000:
                print(f"[GPS REJECTED] Large ENU: e={e:.1f}, n={n:.1f}")
                return False

            u = alt - self.reference_gps[2]
            z = np.array([e, n, u], dtype=float)

            # BUG PRESERVED: hdop used as sigma directly
            R_pos = self.ekf.R
            if pos_sigma is not None and pos_sigma > 0:
                R_pos = np.eye(3) * (pos_sigma ** 2)

            self.ekf.update(
                z,
                HJacobian=H_jacobian_position,
                Hx=hx_position,
                R=R_pos
            )
            self.ekf.x[6:10] = normalize_quaternion(self.ekf.x[6:10])
            return True

        except Exception as exc:
            print(f"[GPS UPDATE ERROR] {exc}")
            self.ekf.P[0:3, 0:3] = np.eye(3) * 100.0
            return False

    def update_heading(self, heading_deg, heading_sigma_deg=2.0):
        """BUG PRESERVED: heading_sigma_deg=2.0 (too small)."""
        if heading_deg is None:
            return False

        yaw_meas = heading_deg_to_yaw_rad(heading_deg)
        R_heading = np.array([[np.deg2rad(heading_sigma_deg) ** 2]])

        try:
            self.ekf.update(
                np.array([yaw_meas], dtype=float),
                HJacobian=H_jacobian_heading,
                Hx=hx_heading,
                R=R_heading,
                residual=angle_residual,
            )
            self.ekf.x[6:10] = normalize_quaternion(self.ekf.x[6:10])
            return True
        except Exception as exc:
            print(f"[HEADING UPDATE ERROR] {exc}")
            return False

    def get_position(self):
        return self.ekf.x[0:3]

    def get_velocity(self):
        return self.ekf.x[3:6]

    def get_yaw_deg(self):
        return np.rad2deg(quaternion_to_yaw(self.ekf.x[6:10])) % 360.0

    def get_euler_deg(self):
        q = normalize_quaternion(self.ekf.x[6:10])
        r = R.from_quat(q)
        return r.as_euler('zyx', degrees=True)

    def get_gps_position(self):
        """BUG PRESERVED: arctan2(x,y) single-geodesic conversion."""
        if self.reference_gps is None:
            return None

        x, y, z = self.ekf.x[0:3]
        flat_distance = np.sqrt(x**2 + y**2)

        if (flat_distance > MAX_EKF_POSITION_RADIUS_M
                or abs(z) > MAX_EKF_ALTITUDE_OFFSET_M):
            print(
                f"[GPS CONVERSION REJECTED] "
                f"x={x:.1f}, y={y:.1f}, z={z:.1f}"
            )
            return None

        try:
            bearing = np.degrees(np.arctan2(x, y)) % 360.0
            ref_point = Point(self.reference_gps[0], self.reference_gps[1])
            dest = geodesic(meters=flat_distance).destination(ref_point, bearing)
            alt_out = self.reference_gps[2] + z
            return dest.latitude, dest.longitude, alt_out
        except Exception as exc:
            print(f"[POSITION CONVERSION ERROR] {exc}")
            return None

    def get_mode_string(self):
        if self.has_gps and self.has_imu:
            return "GPS+IMU fusion"
        elif self.has_imu:
            return "IMU only (relative)"
        elif self.has_gps:
            return "GPS only (no prediction)"
        else:
            return "no sensors"


# ─────────────────────────────────────────────
# IMU PACKET READER — uses ring buffer but
# ORIGINAL fixed format preserved
# ─────────────────────────────────────────────

class IMUPacketReader:
    def __init__(self, ser):
        self.ser = ser
        self._buf = bytearray()

    def _refill(self):
        avail = self.ser.in_waiting
        if avail > 0:
            self._buf.extend(self.ser.read(min(avail, 4096)))

    def read_packet(self):
        self._refill()

        while True:
            sync_idx = -1
            for i in range(len(self._buf) - 1):
                if self._buf[i] == 0xAA and self._buf[i + 1] == 0x55:
                    sync_idx = i
                    break

            if sync_idx < 0:
                if len(self._buf) > 1:
                    self._buf = self._buf[-1:]
                return None, None

            if sync_idx > 0:
                self._buf = self._buf[sync_idx:]

            if len(self._buf) < 3:
                return None, None

            length = self._buf[2]
            total_needed = 3 + length + 2  # original reads 2 CRC bytes
            if len(self._buf) < total_needed:
                return None, None

            payload = bytes(self._buf[3: 3 + length])
            self._buf = self._buf[total_needed:]

            if len(payload) < 4:
                continue

            header = struct.unpack("<I", payload[:4])[0]
            cmd_id = header & 0x7F

            timestamp, data = self._parse_packet(cmd_id, payload[4:])
            if timestamp is not None and data is not None:
                return timestamp, data

    @staticmethod
    def _parse_packet(cmd_id, data):
        try:
            if cmd_id == 41:
                # BUG PRESERVED: fixed format requires exactly 9 floats
                if len(data) < 40:
                    return None, None
                vals = struct.unpack("<Ifff fff fff", data[:40])
                return vals[0], {
                    "packet_type": "RAW",
                    "accel_x": vals[1], "accel_y": vals[2],
                    "accel_z": vals[3],
                    "gyro_x": vals[4], "gyro_y": vals[5],
                    "gyro_z": vals[6],
                    "mag_x": vals[7], "mag_y": vals[8],
                    "mag_z": vals[9],
                }

            if cmd_id == 32:
                if len(data) < 20:
                    return None, None
                vals = struct.unpack("<Iffff", data[:20])
                return vals[0], {
                    "packet_type": "QUAT",
                    "q1": vals[1], "q2": vals[2],
                    "q3": vals[3], "q4": vals[4],
                }

            if cmd_id == 35:
                if len(data) < 16:
                    return None, None
                vals = struct.unpack("<Ifff", data[:16])
                return vals[0], {
                    "packet_type": "RPY",
                    "roll": vals[1], "pitch": vals[2],
                    "yaw": vals[3],
                }

        except struct.error:
            pass

        return None, None


# ─────────────────────────────────────────────
# GPS LINE READER (same as original)
# ─────────────────────────────────────────────

def read_gps_line(ser, last_valid_raw_gps=None):
    try:
        raw_bytes = ser.readline()
        if not raw_bytes:
            return None
        line = raw_bytes.decode("ascii", errors="strict").strip()
    except UnicodeDecodeError:
        return None

    if not line:
        return None

    if PRINT_ALL_NMEA_LINES:
        print(f"[NMEA] {line}")

    if not line.startswith("$"):
        log_raw_nmea(time.time(), line, msg_type="UNKNOWN",
                     parse_status="ignored", note="no_dollar_prefix")
        return None

    if "*" in line and not valid_nmea_checksum(line):
        print(f"[NMEA REJECTED] Bad checksum: {line}")
        log_raw_nmea(time.time(), line, msg_type="UNKNOWN",
                     parse_status="rejected", note="bad_checksum")
        return None

    try:
        msg = pynmea2.parse(line)
        now = time.time()
        msg_type = type(msg).__name__
        log_raw_nmea(now, line, msg_type=msg_type, parse_status="ok")

        if isinstance(msg, pynmea2.types.talker.GGA):
            lat = nmea_to_decimal_degrees(
                msg.lat, msg.lat_dir, is_latitude=True)
            lon = nmea_to_decimal_degrees(
                msg.lon, msg.lon_dir, is_latitude=False)

            alt = None
            if msg.altitude not in (None, ""):
                try:
                    alt = float(msg.altitude)
                except ValueError:
                    alt = None

            hdop = None
            if msg.horizontal_dil not in (None, ""):
                try:
                    hdop = float(msg.horizontal_dil)
                except ValueError:
                    hdop = None

            fix_quality = None
            if msg.gps_qual not in (None, ""):
                try:
                    fix_quality = int(msg.gps_qual)
                except ValueError:
                    fix_quality = None

            num_sats = None
            if msg.num_sats not in (None, ""):
                try:
                    num_sats = int(msg.num_sats)
                except ValueError:
                    num_sats = None

            if PRINT_RAW_GPS_DEBUG:
                print(
                    f"[GGA] lat={lat} lon={lon} alt={alt} "
                    f"fix={fix_quality} sats={num_sats} hdop={hdop}"
                )

            if lat is None or lon is None:
                log_raw_nmea(now, line, msg_type="GGA",
                             parse_status="rejected",
                             note="lat_lon_parse_failed")
                return None

            if fix_quality is None or fix_quality < MIN_FIX_QUALITY:
                return {
                    "type": "GGA", "valid": False,
                    "lat": lat, "lon": lon, "alt": alt,
                    "hdop": hdop, "fix_quality": fix_quality,
                    "num_sats": num_sats, "raw_line": line,
                }

            if last_valid_raw_gps is not None:
                try:
                    jump_m = geodesic(
                        (last_valid_raw_gps["lat"],
                         last_valid_raw_gps["lon"]),
                        (lat, lon)
                    ).meters
                    if jump_m > MAX_RAW_GPS_JUMP_M:
                        print(f"[GGA REJECTED] Jump {jump_m:.1f} m")
                        log_raw_nmea(
                            now, line, msg_type="GGA",
                            parse_status="rejected",
                            note=f"jump_{jump_m:.1f}m")
                        return {
                            "type": "GGA", "valid": False,
                            "lat": lat, "lon": lon, "alt": alt,
                            "hdop": hdop, "fix_quality": fix_quality,
                            "num_sats": num_sats, "raw_line": line,
                        }
                except Exception:
                    pass

            return {
                "type": "GGA", "valid": True,
                "lat": lat, "lon": lon, "alt": alt,
                "hdop": hdop, "fix_quality": fix_quality,
                "num_sats": num_sats, "raw_line": line,
            }

        if isinstance(msg, pynmea2.types.talker.GST):
            lat_err = (float(msg.data[5])
                       if len(msg.data) > 5 and msg.data[5] else None)
            lon_err = (float(msg.data[6])
                       if len(msg.data) > 6 and msg.data[6] else None)
            alt_err = (float(msg.data[7])
                       if len(msg.data) > 7 and msg.data[7] else None)
            print(
                f"[GST] Lat err: {lat_err} m | "
                f"Lon err: {lon_err} m | Alt err: {alt_err} m"
            )
            return {
                "type": "GST",
                "lat_err": lat_err, "lon_err": lon_err,
                "alt_err": alt_err, "raw_line": line,
            }

        if isinstance(msg, pynmea2.types.talker.HDT):
            heading = float(msg.heading) if msg.heading else None
            print(f"[HDT] Heading: {heading} True")
            return {
                "type": "HDT", "heading_deg": heading,
                "raw_line": line,
            }

        return None

    except pynmea2.ParseError as exc:
        log_raw_nmea(time.time(), line, msg_type="UNKNOWN",
                     parse_status="parse_error", note=str(exc))
        return None
    except Exception as exc:
        log_raw_nmea(time.time(), line, msg_type="UNKNOWN",
                     parse_status="exception", note=str(exc))
        return None


# ─────────────────────────────────────────────
# MAIN
# ─────────────────────────────────────────────

def main():
    ekf = SensorFusionEKF()

    imu_ser = None
    gps_ser = None
    imu_reader = None

    if IMU_ENABLED:
        try:
            imu_ser = serial.Serial(IMU_PORT, IMU_BAUD, timeout=0)
            imu_reader = IMUPacketReader(imu_ser)
            print(f"Opened IMU: {IMU_PORT} @ {IMU_BAUD}")
        except serial.SerialException as exc:
            print(f"[WARNING] Could not open IMU port: {exc}")

    if GPS_ENABLED:
        try:
            gps_ser = serial.Serial(GPS_PORT, GPS_BAUD, timeout=0.01)
            print(f"Opened GPS: {GPS_PORT} @ {GPS_BAUD}")
        except serial.SerialException as exc:
            print(f"[WARNING] Could not open GPS port: {exc}")

    if imu_ser is None and gps_ser is None:
        print("[ERROR] No sensors available. Exiting.")
        return

    last_gps_error = None
    last_heading_deg = None
    last_valid_raw_gps = None

    # ── Phase 1: Wait for GPS (with IMU-only fallback) ──
    if gps_ser is not None:
        print(f"Waiting for GPS fix ({GPS_FIX_TIMEOUT_S}s timeout)...")
        start_time = time.time()

        while (ekf.reference_gps is None
               and time.time() - start_time < GPS_FIX_TIMEOUT_S):

            gps_fix = read_gps_line(
                gps_ser, last_valid_raw_gps=last_valid_raw_gps)

            if not gps_fix:
                if imu_reader is not None:
                    for _ in range(50):
                        ts, data = imu_reader.read_packet()
                        if data is None:
                            break
                time.sleep(0.001)
                continue

            if gps_fix.get("type") == "GST":
                last_gps_error = (
                    gps_fix.get("lat_err"),
                    gps_fix.get("lon_err"),
                    gps_fix.get("alt_err"),
                )
            elif gps_fix.get("type") == "HDT":
                last_heading_deg = gps_fix.get("heading_deg")
            elif (gps_fix.get("type") == "GGA"
                  and gps_fix.get("valid")):
                lat = gps_fix["lat"]
                lon = gps_fix["lon"]
                alt = gps_fix.get("alt")
                last_valid_raw_gps = gps_fix.copy()

                log_position_and_error(
                    time.time(),
                    (lat, lon, alt if alt is not None else 0.0),
                    position_source="raw_gps",
                    gps_error=last_gps_error,
                    heading_deg=last_heading_deg,
                    raw_gps_lat=lat, raw_gps_lon=lon,
                    raw_gps_alt=alt,
                    raw_fix_quality=gps_fix.get("fix_quality"),
                    raw_hdop=gps_fix.get("hdop"),
                )

                if lat and lon and lat != 0.0 and lon != 0.0:
                    ekf.set_reference_gps(lat, lon, alt)
                    print(f"Reference GPS set: {ekf.reference_gps}")

        if ekf.reference_gps is None:
            print("[WARNING] No GPS fix. Continuing in IMU-only mode.")
    else:
        print("GPS disabled. Running in IMU-only mode.")

    print(f"Operating mode: {ekf.get_mode_string()}")
    print("Starting sensor fusion loop (CODE B — bugs preserved)...")

    # ── Phase 2: Main loop ──
    imu_packets_read = 0
    imu_predictions_done = 0
    gps_lines_read = 0
    loop_counter = 0
    rate_window_start = time.time()
    last_predict_print_time = 0.0
    last_imu_log_time = 0.0
    IMU_LOG_INTERVAL = 0.05

    try:
        while True:
            loop_counter += 1

            imu_batch_count = 0
            if imu_reader is not None:
                while True:
                    imu_timestamp, imu_data = imu_reader.read_packet()
                    if imu_data is None:
                        break

                    imu_packets_read += 1
                    imu_batch_count += 1

                    if imu_data.get("packet_type") == "RAW":
                        gyro = np.array([
                            imu_data["gyro_x"],
                            imu_data["gyro_y"],
                            imu_data["gyro_z"],
                        ], dtype=float)
                        accel = np.array([
                            imu_data["accel_x"],
                            imu_data["accel_y"],
                            imu_data["accel_z"],
                        ], dtype=float)

                        predicted = ekf.update_imu(
                            gyro, accel, imu_timestamp)

                        if predicted:
                            imu_predictions_done += 1
                            now = time.time()

                            if now - last_predict_print_time > 0.25:
                                pos_local = ekf.get_position()
                                vel = ekf.get_velocity()

                                gps_pos = ekf.get_gps_position()
                                if gps_pos is not None:
                                    yaw_deg = ekf.get_yaw_deg()
                                    print(
                                        f"[IMU] "
                                        f"lat={gps_pos[0]:.8f} "
                                        f"lon={gps_pos[1]:.8f} "
                                        f"alt={gps_pos[2]:.2f} "
                                        f"yaw={yaw_deg:.1f} | "
                                        f"local=("
                                        f"{pos_local[0]:.2f},"
                                        f"{pos_local[1]:.2f},"
                                        f"{pos_local[2]:.2f}) "
                                        f"vel=("
                                        f"{vel[0]:.2f},"
                                        f"{vel[1]:.2f},"
                                        f"{vel[2]:.2f})"
                                    )
                                else:
                                    euler = ekf.get_euler_deg()
                                    print(
                                        f"[IMU-ONLY] "
                                        f"pos=("
                                        f"{pos_local[0]:.3f},"
                                        f"{pos_local[1]:.3f},"
                                        f"{pos_local[2]:.3f}) m | "
                                        f"vel=("
                                        f"{vel[0]:.3f},"
                                        f"{vel[1]:.3f},"
                                        f"{vel[2]:.3f}) m/s | "
                                        f"yaw={euler[0]:.1f} "
                                        f"pitch={euler[1]:.1f} "
                                        f"roll={euler[2]:.1f}"
                                    )
                                last_predict_print_time = now

                            if now - last_imu_log_time > IMU_LOG_INTERVAL:
                                gps_pos = ekf.get_gps_position()
                                log_position_and_error(
                                    now, gps_pos,
                                    position_source="imu_prediction",
                                    gps_error=last_gps_error,
                                    heading_deg=last_heading_deg,
                                    raw_gps_lat=(
                                        last_valid_raw_gps["lat"]
                                        if last_valid_raw_gps
                                        else None),
                                    raw_gps_lon=(
                                        last_valid_raw_gps["lon"]
                                        if last_valid_raw_gps
                                        else None),
                                    raw_gps_alt=(
                                        last_valid_raw_gps.get("alt")
                                        if last_valid_raw_gps
                                        else None),
                                    raw_fix_quality=(
                                        last_valid_raw_gps.get(
                                            "fix_quality")
                                        if last_valid_raw_gps
                                        else None),
                                    raw_hdop=(
                                        last_valid_raw_gps.get("hdop")
                                        if last_valid_raw_gps
                                        else None),
                                    imu_dt=ekf.dt,
                                    ekf_pos_local=ekf.get_position(),
                                    ekf_vel_local=ekf.get_velocity(),
                                )
                                last_imu_log_time = now

                    if imu_batch_count > 800:
                        break

            if gps_ser is not None:
                gps_fix = read_gps_line(
                    gps_ser,
                    last_valid_raw_gps=last_valid_raw_gps,
                )

                if gps_fix is not None:
                    gps_lines_read += 1
                    msg_type = gps_fix.get("type")

                    if msg_type == "GGA":
                        lat = gps_fix.get("lat")
                        lon = gps_fix.get("lon")
                        alt = gps_fix.get("alt")

                        if lat is not None and lon is not None:
                            log_position_and_error(
                                time.time(),
                                (lat, lon,
                                 alt if alt is not None else 0.0),
                                position_source="raw_gps",
                                gps_error=last_gps_error,
                                heading_deg=last_heading_deg,
                                raw_gps_lat=lat,
                                raw_gps_lon=lon,
                                raw_gps_alt=alt,
                                raw_fix_quality=gps_fix.get(
                                    "fix_quality"),
                                raw_hdop=gps_fix.get("hdop"),
                                imu_dt=ekf.dt,
                                ekf_pos_local=ekf.get_position(),
                                ekf_vel_local=ekf.get_velocity(),
                            )

                        if gps_fix.get("valid"):
                            last_valid_raw_gps = gps_fix.copy()

                            if ekf.reference_gps is None:
                                if (lat and lon
                                        and lat != 0.0
                                        and lon != 0.0):
                                    ekf.set_reference_gps(
                                        lat, lon, alt)
                                    print(
                                        f"[LATE GPS INIT] "
                                        f"Reference: "
                                        f"{ekf.reference_gps}"
                                    )

                            if ekf.reference_gps is not None:
                                hdop = gps_fix.get("hdop")
                                pos_sigma = (hdop
                                             if hdop is not None
                                             and hdop > 0
                                             else None)
                                gps_updated = ekf.update_gps(
                                    lat, lon, alt,
                                    pos_sigma=pos_sigma,
                                )

                                if gps_updated:
                                    gps_pos = ekf.get_gps_position()
                                    if gps_pos is not None:
                                        yaw_deg = ekf.get_yaw_deg()
                                        print(
                                            f"[GPS UPDATE] "
                                            f"lat="
                                            f"{gps_pos[0]:.8f} "
                                            f"lon="
                                            f"{gps_pos[1]:.8f} "
                                            f"alt="
                                            f"{gps_pos[2]:.2f} "
                                            f"yaw={yaw_deg:.1f}"
                                        )
                                        log_position_and_error(
                                            time.time(),
                                            gps_pos,
                                            position_source=(
                                                "gps_update"),
                                            gps_error=(
                                                last_gps_error),
                                            heading_deg=(
                                                last_heading_deg),
                                            raw_gps_lat=lat,
                                            raw_gps_lon=lon,
                                            raw_gps_alt=alt,
                                            raw_fix_quality=(
                                                gps_fix.get(
                                                    "fix_quality")),
                                            raw_hdop=(
                                                gps_fix.get(
                                                    "hdop")),
                                            imu_dt=ekf.dt,
                                            ekf_pos_local=(
                                                ekf.get_position()),
                                            ekf_vel_local=(
                                                ekf.get_velocity()),
                                        )

                    elif msg_type == "GST":
                        last_gps_error = (
                            gps_fix.get("lat_err"),
                            gps_fix.get("lon_err"),
                            gps_fix.get("alt_err"),
                        )

                    elif msg_type == "HDT":
                        last_heading_deg = gps_fix.get("heading_deg")
                        if last_heading_deg is not None:
                            heading_updated = ekf.update_heading(
                                last_heading_deg,
                                heading_sigma_deg=2.0,
                            )
                            if heading_updated:
                                gps_pos = ekf.get_gps_position()
                                if gps_pos is not None:
                                    print(
                                        f"[HDT UPDATE] "
                                        f"lat="
                                        f"{gps_pos[0]:.8f} "
                                        f"lon="
                                        f"{gps_pos[1]:.8f} "
                                        f"yaw="
                                        f"{ekf.get_yaw_deg():.1f}"
                                    )
                                    log_position_and_error(
                                        time.time(),
                                        gps_pos,
                                        position_source=(
                                            "heading_update"),
                                        gps_error=(
                                            last_gps_error),
                                        heading_deg=(
                                            last_heading_deg),
                                        raw_gps_lat=(
                                            last_valid_raw_gps[
                                                "lat"]
                                            if last_valid_raw_gps
                                            else None),
                                        raw_gps_lon=(
                                            last_valid_raw_gps[
                                                "lon"]
                                            if last_valid_raw_gps
                                            else None),
                                        raw_gps_alt=(
                                            last_valid_raw_gps.get("alt")
                                            if last_valid_raw_gps
                                            else None),
                                        raw_fix_quality=(
                                            last_valid_raw_gps.get("fix_quality")
                                            if last_valid_raw_gps
                                            else None),
                                        raw_hdop=(
                                            last_valid_raw_gps.get("hdop")
                                            if last_valid_raw_gps
                                            else None),
                                        imu_dt=ekf.dt,
                                        ekf_pos_local=(
                                            ekf.get_position()),
                                        ekf_vel_local=(
                                            ekf.get_velocity()),
                                    )

            # ── Rate reporting ──
            now = time.time()
            if now - rate_window_start >= 2.0:
                elapsed = now - rate_window_start
                mode = ekf.get_mode_string()
                parts = [f"[RATE] mode={mode}"]

                if imu_reader is not None:
                    parts.append(
                        f"imu={imu_packets_read / elapsed:.0f}/s")
                    parts.append(
                        f"predictions="
                        f"{imu_predictions_done / elapsed:.0f}/s")
                    parts.append(
                        f"skipped={ekf.skipped_predicts}")
                    parts.append(f"dt={ekf.dt:.6f}")

                if gps_ser is not None:
                    parts.append(
                        f"gps={gps_lines_read / elapsed:.0f}/s")

                parts.append(
                    f"loops={loop_counter / elapsed:.0f}/s")

                print(" | ".join(parts))

                loop_counter = 0
                imu_packets_read = 0
                imu_predictions_done = 0
                gps_lines_read = 0
                rate_window_start = now

            if imu_batch_count == 0:
                if imu_reader is not None:
                    time.sleep(0.0002)
                else:
                    time.sleep(0.01)

    except KeyboardInterrupt:
        print("\nShutting down...")
    finally:
        position_logger.flush()
        nmea_logger.flush()
        if imu_ser is not None:
            imu_ser.close()
        if gps_ser is not None:
            gps_ser.close()
        print(f"Closed. Final mode: {ekf.get_mode_string()}")


if __name__ == "__main__":
    main()