...
# =========================================================
# SHARED PULSE EPOCH  (puts the AGX/CV7 in the nanos' pulse loop)
# ---------------------------------------------------------
# The nanos latch the PPS edge and record every image's timestamp as
#   T_from_pulse = get_tow_from_utc() - event_time
# where event_time is the absolute GPS Time-of-Week at that pulse.
# remote_display_raw_bytes.py (running on this same AGX) recovers event_time
# from the incoming image EXIF and writes it to PULSE_EPOCH_FILE. We read it
# here and stamp every CV7 row with the SAME time-since-pulse, so CV7 data and
# images share one origin. get_tow_from_utc() is byte-identical to the nano's.
# =========================================================
import os as _os_pulse

PULSE_EPOCH_FILE = _os_pulse.environ.get("PULSE_EPOCH_FILE", "/tmp/pulse_epoch.json")
_GPS_EPOCH = datetime(1980, 1, 6, tzinfo=timezone.utc)
_pulse_event_time = None          # cached absolute pulse TOW, once available
_pulse_last_check = 0.0

def get_tow_from_utc() -> float:
    """Absolute GPS Time-of-Week from the system clock (matches the nanos exactly)."""
    now = datetime.now(timezone.utc)
    gps_seconds = (now - _GPS_EPOCH).total_seconds()
    return round(gps_seconds % (7 * 86400), 4)

def get_pulse_event_time():
    """Return the shared pulse epoch (event_time), polling the file until it appears."""
    global _pulse_event_time, _pulse_last_check
    if _pulse_event_time is not None:
        return _pulse_event_time
    now = time.monotonic()
    if now - _pulse_last_check < 0.5:   # avoid hammering the disk every row
        return None
    _pulse_last_check = now
    try:
        with open(PULSE_EPOCH_FILE, "r") as fh:
            data = json.load(fh)
        et = float(data["event_time"])
        _pulse_event_time = et
        print(f"[pulse-epoch] CV7 locked shared event_time={et} from {PULSE_EPOCH_FILE}")
        return et
    except (FileNotFoundError, KeyError, ValueError, json.JSONDecodeError):
        return None

def time_from_pulse():
    """Seconds since the shared pulse, matching the images' T_from_pulse; '' if unknown."""
    et = get_pulse_event_time()
    if et is None:
        return ""
...
def finalize_event(
    ...
    row["T_from_pulse"] = time_from_pulse()   # shared pulse epoch (same origin as images)
    if mip_fields:
