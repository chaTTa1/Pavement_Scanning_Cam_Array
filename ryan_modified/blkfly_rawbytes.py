...
PPS_VALID_WINDOW_NS = 1200000000
...
def connect_log_socket():
    """Connects the camera computer to the AGX statistics socket."""
    global log_sock
    try:
...
def remote_print(msg):
    """Prints a message locally and forwards it to the AGX when connected."""
    print(msg)
    if log_sock is None:
        return
    try:
...
def stats_thread(stop_event):
    """Reports the capture, processing, and network rates once per second."""
...
def drop_oldest_and_put(q, item, label):
    """Adds an item to a bounded queue and drops the oldest item if required."""
    try:
        q.put_nowait(item)
        return True
    except queue.Full:
...
def pps_listener(pps_pin, stop_event):
    """
    Records every PPS GPIO edge and assigns the shared session sequence.

    A START_ON_NEXT_PPS command arms a new session. The first PPS after that
    command becomes sequence zero, and every later PPS increments the sequence.
    """
    global pps_state
    ...
    while not stop_event.is_set():
        ...
        with pps_lock:
            armed_session_id = pps_state["armed_session_id"]
            previous_pps_monotonic_ns = pps_state["monotonic_ns"]
            ...
            if armed_session_id is not None:
                pps_state["active_session_id"] = armed_session_id
                pps_state["armed_session_id"] = None
                pps_state["sequence"] = 0
            elif pps_state["active_session_id"] is not None:
                pps_state["sequence"] += sequence_step

            pps_state["monotonic_ns"] = pps_monotonic_ns
            pps_state["interval_ns"] = pps_interval_ns
            active_session_id = pps_state["active_session_id"]
            sequence = pps_state["sequence"]

            if active_session_id is not None:
                pps_state["history"].append({
                    "session_id": active_session_id,
                    "pps_sequence": sequence,
                    "monotonic_ns": pps_monotonic_ns,
                    "pps_interval_ns": pps_interval_ns,
                    "missed_pps_count": sequence_step - 1,
                    })
                pps_state["history"] = pps_state["history"][-4:]

        if active_session_id is None:
            print("[pps] Pulse received while waiting for an AGX session")
        else:
            print(f"[pps] session={active_session_id}, sequence={sequence}")


def control_listener(stop_event):
    """
    Receives AGX session commands and acknowledges that this Nano is armed.

    Repeated commands for the active or already armed session are idempotent,
    so a delayed UDP retry cannot reset a session that has already started.
    """
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    sock.bind(('', CONTROL_PORT))
    sock.settimeout(1.0)
    print(f"[control] Listening for AGX commands on UDP port {CONTROL_PORT}")
    ...
    try:
        while not stop_event.is_set():
            ...
            try:
                ...
                print(f"[control] Invalid command: {ex}")
                continue

            if message.get("command") != "START_ON_NEXT_PPS":
                continue

            session_id = message.get("session_id")
            if not session_id:
                print("[control] START_ON_NEXT_PPS missing session_id")
                continue

            # The AGX may begin synchronization while this Nano is still
            # initializing its Blackfly. Do not acknowledge readiness until
            # acquisition has actually started. The AGX retries the command,
            # so a later retry will complete the handshake without changing
            # the existing asynchronous capture and streaming pipeline.
            if not camera_ready_event.is_set():
                print("[control] Camera is not ready; deferring ARMED reply")
                continue
            ...
            with pps_lock:
                ...
                if session_id != active_session_id and session_id != armed_session_id:
                    pps_state["armed_session_id"] = session_id
            ...
            sock.sendto(
                ...
                (address[0], CONTROL_ACK_PORT),
                )
            print(f"[control] ARMED for session {session_id}")
    finally:
        sock.close()


def get_frame_timing(frame_id, image_monotonic_ns):
    """
    Builds the PPS timing metadata associated with one camera frame.

    The timestamp remains invalid until the first synchronized PPS is received.
    A frame is also invalidated when its measured delay is outside the expected
    one-second PPS interval plus the configured tolerance.
    """
    with pps_lock:
        eligible_pps_records = [
            record for record in pps_state["history"]
            if record["monotonic_ns"] <= image_monotonic_ns
            ]

    if eligible_pps_records:
        pps_record = max(
            eligible_pps_records,
            key=lambda record: record["monotonic_ns"],
            )
        session_id = pps_record["session_id"]
        pps_sequence = pps_record["pps_sequence"]
        pps_monotonic_ns = pps_record["monotonic_ns"]
        pps_interval_ns = pps_record["pps_interval_ns"]
        missed_pps_count = pps_record["missed_pps_count"]
    else:
        session_id = None
        pps_sequence = None
        pps_monotonic_ns = None
        pps_interval_ns = None
        missed_pps_count = None

    timestamp_valid = (
        session_id is not None
        and pps_sequence is not None
        and pps_monotonic_ns is not None
        )

    if timestamp_valid:
        delta_from_pps_ns = image_monotonic_ns - pps_monotonic_ns
        timestamp_valid = 0 <= delta_from_pps_ns < PPS_VALID_WINDOW_NS
    else:
        delta_from_pps_ns = None
    ...
    return {
        ...
        "delta_from_pps_ns": delta_from_pps_ns,
        "pps_interval_ns": pps_interval_ns,
        "missed_pps_count": missed_pps_count,
...
def send_framed_payload(sock, metadata_bytes, image_bytes, exif_bytes):
    """
    Sends one metadata, raw image, and EXIF message over the TCP stream.

    Each byte section is prefixed with a four-byte big-endian length. Socket
    errors are allowed to reach broadcast_thread so it can reconnect.
    """
    sock.sendall(struct.pack("!I", len(metadata_bytes)))
    sock.sendall(struct.pack("!I", len(image_bytes)))
    sock.sendall(struct.pack("!I", len(exif_bytes)))
    sock.sendall(metadata_bytes)
    sock.sendall(image_bytes)
    sock.sendall(exif_bytes)


def broadcast_thread(stream_q, stop_event):
    """
    Sends processed frames and their PPS metadata to the AGX over TCP.

    A failed send closes the current connection, reconnects, and retries the
    current frame once without reporting a false successful frame count.
    """
    global sent_frames
    sock = None
    ...
    try:
        ...
        while not stop_event.is_set() or not stream_q.empty():
            ...
            frame_id, image_bytes, exif_bytes, metadata = stream_item
...
def gps_listener(udp_port=5005):
    """Receives the latest per-camera GPS position broadcast over UDP."""
...
def decimal_to_dms_precise(decimal):
    """Converts decimal degrees into degree, minute, and second components."""
...
def get_tow_from_utc():
    """Estimates GPS time of week from the current host UTC clock."""
...
def exif_bytes(make, model, comment, focal):
    ...
    def to_rational(value):
        """Converts a numeric EXIF value into a numerator and denominator."""
...
def embed_tow_with_exif_module(image_path, tow_value, latest_gps):
    """Writes a relative time and the latest available GPS values into a JPEG."""
...
def capture_image(cam, frame_id, timeout=.5, save_path=None, return_array=True):
    """
    Retrieves one image and records when Spinnaker makes the frame available.

    The monotonic timestamp is taken immediately after GetNextImage returns and
    before GetNDArray copies the image into a NumPy array.
    """
    image_result = cam.GetNextImage(int(timeout * 1000))
    image_monotonic_ns = time.monotonic_ns()
    ...
    try:
        ...
        if return_array:
            ...
            packet = (frame_id, image_monotonic_ns, frame)
            image_array = frame
        else:
            packet = None
            image_array = None
...
def cam_configuration(nodemap,
                      ...
                      verbose=True):
    """
    Configures camera acquisition, exposure, gain, format, ROI, and buffers.
    """
...
def image_proc_thread(save_q, time_q, savedir, stop_event):
    """
    Converts frames to raw bytes and attaches PPS timing metadata and EXIF.

    The metadata is carried as a separate TCP section and is also copied into
    the EXIF user comment so the saved image retains the timing information.
    """
    ...
    while not stop_event.is_set() or not save_q.empty() or not time_q.empty():
        ...
        try:
            if isinstance(payload, tuple) and len(payload) == 2 and isinstance(payload[0], tuple):
                packet, frame_metadata = payload
            else:
                packet = payload
                frame_metadata = time_q.get(timeout=0.5)

            frame_id, image_monotonic_ns, frame = packet
            frame_comment = json.dumps(
                frame_metadata,
                separators=(',', ':'),
                )
            ...
            exif_byte = exif_bytes('Flir', 'BlackFly', frame_comment, 6)
            #print(len(exif_byte))
            #print(len(final_packet))
            drop_oldest_and_put(
                stream_q,
                (frame_id, final_packet, exif_byte, frame_metadata),
                "stream",
                )
            with stats_lock:
                encoded_frames += 1
...
def acquire_images(cam,
    ...
    try:
        while not stop_event.is_set():
            ...
            frame_id, image_monotonic_ns, frame = packet
            frame_metadata = get_frame_timing(frame_id, image_monotonic_ns)
            drop_oldest_and_put(save_q, (packet, frame_metadata), "save")
            with stats_lock:
                captured_frames += 1
    ...
    # The PPS times the Nano and does not change the free-running camera mode.
    if triggerType.lower() == "off":
        cam.EndAcquisition()
        return result
...
def print_device_info(nodemap_tldevice):
    """Prints the camera transport-layer device information."""
    print('\n*** DEVICE INFORMATION ***\n')
    try:
        result = True
...
def run_single_camera(cam,
                      ...
                      bufferCount=30,
                      timeout=10):
    """Initializes one camera, runs acquisition, and safely deinitializes it."""
    result = True
    try:
        cam.Init()
...
def sysScan():
    """Discovers connected Spinnaker cameras and returns the system handles."""
    result = True
...
def clearDir(targetDir):
    """Removes existing files from the requested image output directory."""
...
def get_IEnumeration_node_current_entry_name(nodemap, nodename, verbose=True):
    """Reads and optionally prints the current entry of an enumeration node."""
...
def get_IInteger_node_current_val(nodemap, nodename, verbose=True):
    """Reads and optionally prints the current integer node value."""
...
def get_IFloat_node_current_val(nodemap, nodename, verbose=True):
    """Reads and optionally prints the current floating-point node value."""
...
def get_IString_node_current_str(nodemap, nodename, verbose=True):
    """Reads and optionally prints the current string node value."""
...
def get_IBoolean_node_current_val(nodemap, nodename, verbose=True):
    """Reads and optionally prints the current Boolean node value."""
...
def enableFrameRateSetting(nodemap):
    """Enables manual camera frame-rate control and disables automatic control."""
...
def setPixelFormat(nodemap):
    """Sets the camera output pixel format to Mono8."""
...
def setResolution(nodemap):
    """Sets a zero-offset 720 by 540 camera region of interest."""
...
def setFrameRate(nodemap, frameRate):
    """Sets the requested manual acquisition frame rate."""
...
def enableExposureAuto(nodemap):
    """Enables continuous automatic camera exposure."""
...
def disableExposureAuto(nodemap):
    """Disables automatic exposure so the configured exposure is used."""
    ptrExposureAuto = PySpin.CEnumerationPtr(nodemap.GetNode("ExposureAuto"))
    if (not PySpin.IsAvailable(ptrExposureAuto)) or (not PySpin.IsWritable(ptrExposureAuto)):
        print('Unable to retrieve ExposureAuto. Aborting...')
        return False
...
def disableExposureCompensationAuto(nodemap):
    """Disables automatic exposure compensation when the node is available."""
...
def setExposureMode(nodemap, exposureModeToSet):
    """Sets the requested Spinnaker exposure mode."""
...
def setTriggerMode(nodemap, TriggerModeToSet):
    """Turns the camera trigger mode on or off."""
...
def setTriggerActivation(nodemap, TriggerActivationToSet):
    """Sets which signal edge activates the camera trigger input."""
...
def setTriggerOverlap(nodemap, TriggerOverlapToSet):
    """Sets how a camera trigger may overlap an existing acquisition."""
...
def setTriggerSelector(nodemap, TriggerSelectorToSet):
    """Selects the camera event controlled by the trigger."""
...
def setTriggerSource(nodemap, TriggerSourceToSet):
    """Selects the software or hardware source used by camera triggering."""
...
def setExposureTime(nodemap, exposureTime=None):
    """Sets a manual exposure time within the camera-supported limits."""
...
def setAcquisitionMode(nodemap, AcquisitionModeName):
    """Sets the requested camera acquisition mode."""
...
def setStreamBufferHandlingMode(s_node_map, StreamBufferHandlingModeName):
    """Sets how Spinnaker selects frames from the host stream buffer."""
...
def setBufferCount(s_node_map, bufferCount):
    """Sets the number of manually allocated Spinnaker stream buffers."""
...
def disableGainAuto(nodemap):
    """Disables automatic gain so a configured manual gain can be used."""
...
def setGain(nodemap, gain):
    """Sets a manual gain within the camera-supported limits."""
...
def setBlackLevel(nodemap, blackLevel):
    """Sets the requested camera black-level correction value."""
...
def setExposureCompensation(nodemap, pgrExposureCompensation):
    """Sets the requested camera exposure-compensation value."""
...
def trigger_configuration(nodemap, s_node_map, triggerType, verbose=True):
    """Configures camera nodes for free-running, software, or hardware trigger."""
    print('\n*** CONFIGURING TRIGGER ***\n')
    if triggerType == 'software':
...
def activate_trigger(nodemap):
    """Enables the configured camera trigger mode."""
    result = setTriggerMode(nodemap, "On")
    return result


def deactivate_trigger(nodemap):
    """Disables the camera trigger mode."""
    result = setTriggerMode(nodemap, "Off")
    return result


def print_camera_config(nodemap, s_node_map):
    """Prints the current camera acquisition and stream configuration."""
...
def print_trigger_config(nodemap, s_node_map, triggerType="software"):
    """Prints camera trigger nodes relevant to the requested trigger type."""
    if triggerType == 'software':
...
def main():
    """
    Starts GPIO timing, AGX control, image processing, and camera acquisition.

    GPIO remains configured for the full process lifetime and is cleaned only
    after all acquisition and networking threads have been asked to stop.
    """
    global system, x
    ...
    try:
        ...
        # PPS timing now runs continuously in pps_listener.
        pass
        ...
        # GPIO remains active so the PPS thread can listen during acquisition.
        pass
    ...
    try:
        ...
        stats_thread_obj.join(timeout=2)
