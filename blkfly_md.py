...
def broadcast_thread(stream_q, stop_event):
    ...
    try:
        while not stop_event.is_set() or not stream_q.empty():
            ...
            try:
                total_len = len(final_packet)
                seq = 0
                for offset in range(0, total_len, CHUNK_SIZE):
                    chunk = final_packet[offset:offset+CHUNK_SIZE]
                    header = struct.pack("!III", seq, total_len, offset)
                    sock.sendto(header+chunk, (TARGET_IP, TARGET_PORT))
                else:
                    print(f"Frame {frame_id if frame_id is not None else '?'} too large to send via UDP")
            except OSError as e:
...
def acquire_images(cam,
    ...
    try:
        ...
        gps_thread.join()              # wait for acquisition thread            # wait for saving thread
