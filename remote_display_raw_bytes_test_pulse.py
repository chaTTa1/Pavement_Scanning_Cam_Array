...
# =========================================================
# SHARED PULSE EPOCH  (puts the AGX in the nanos' pulse loop)
# ---------------------------------------------------------
# The nanos latch the PPS edge and stamp each image's EXIF with
#   T_from_pulse=<offset>;pulse_tow=<event_time>
# where event_time is the absolute GPS Time-of-Week at the pulse.
# The AGX has no PPS wire, so it recovers that pulse origin here and
# writes it to a local file that CV7_read_nmea.py also reads. Both AGX
# processes therefore share ONE pulse epoch, identical to the images.
# =========================================================
PULSE_EPOCH_FILE = os.environ.get("PULSE_EPOCH_FILE", "/tmp/pulse_epoch.json")
_pulse_epoch_lock = threading.Lock()
_pulse_epoch_published = False

def extract_pulse_tow(exif_bytes):
    """Scan raw EXIF bytes for 'pulse_tow=<float>' and return it, else None."""
    if not exif_bytes:
        return None
    key = b"pulse_tow="
    i = exif_bytes.find(key)
    if i == -1:
        return None
    j = i + len(key)
    num = bytearray()
    while j < len(exif_bytes) and exif_bytes[j:j+1] in b"0123456789.":
        num += exif_bytes[j:j+1]
        j += 1
    try:
        return float(num.decode("ascii"))
    except ValueError:
        return None

def publish_pulse_epoch(exif_bytes):
    """Write the shared pulse epoch once per run (first frame that carries it)."""
    global _pulse_epoch_published
    if _pulse_epoch_published:
        return
    tow = extract_pulse_tow(exif_bytes)
    if tow is None:
        return
    with _pulse_epoch_lock:
        if _pulse_epoch_published:
            return
        payload = {"event_time": tow, "source": "blkfly_exif",
                   "published_unix": time.time()}
        tmp = PULSE_EPOCH_FILE + ".tmp"
        with open(tmp, "w") as fh:
            json.dump(payload, fh)
        os.replace(tmp, PULSE_EPOCH_FILE)   # atomic
        _pulse_epoch_published = True
        print(f"[pulse-epoch] published event_time={tow} -> {PULSE_EPOCH_FILE}")
...
def receiver(server_sock, label):
    global latest_left, latest_mid, latest_right, l_rec, m_rec, r_rec, L_tot_fps, M_tot_fps, R_tot_fps, R_send, R_capt, M_capt, M_send, L_capt, L_send
    frame_count = 0
    last_time = time.time()
    ...
    while not stop_event.is_set():
        ...
        try:
            while not stop_event.is_set():
                ...
                #print(f"{label}: img={img_len}, exif={exif_len}")
                if img_len is None or exif_len is None:
                    break


                img_bytes = recv_exact(conn, img_len)
                exif_bytes = recv_exact(conn, exif_len)
                payload = [img_bytes, exif_bytes]
                if payload is None:
                    break
                ...
                with frame_lock:
                    if label == "left":
                        latest_left = img_bytes 
                    elif label == "mid":
                        latest_mid = img_bytes
                    else:
                        latest_right = img_bytes
...
def decoder(label):
    ...
    while not stop_event.is_set():
        try:
            ...
            img_bytes, exif_bytes = payload
        except queue.Empty:
            continue
        frame = np.frombuffer(img_bytes, dtype=np.uint8)
        frame = frame.reshape((540,720))
        #frame = cv2.imdecode(payload, cv2.IMREAD_GRAYSCALE)
        ...
        if not save_q[label].full():
            save_q[label].put([frame, exif_bytes])
    ...
    while not stop_event.is_set():
        ...
        for label in ["left", "mid", "right"]:
            try:
                ...
                continue
            frame, exif_bytes = payload
            publish_pulse_epoch(exif_bytes)   # share pulse epoch with CV7_read_nmea.py
            made_progress = True
            i = counters[label]
    ...
    try:
        while running and not stop_event.is_set():
            ...
            if left_img is not None:
                left_img = np.frombuffer(left_img, dtype=np.uint8)
                left_img = left_img.reshape((540, 720))
            ...
            if mid_img is not None:
                mid_img = np.frombuffer(mid_img, dtype=np.uint8)
                mid_img = mid_img.reshape((540, 720))
            ...
            if right_img is not None:
                right_img = np.frombuffer(right_img, dtype=np.uint8)
                right_img = right_img.reshape((540, 720))
